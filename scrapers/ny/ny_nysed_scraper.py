"""
New York State Education Department — Grants Scraper
Source: https://www.nysed.gov/funding-opportunities/grants

Scrapes NYSED education, early childhood, workforce literacy, and
library grant programs, follows each detail page, runs Azure AI
enrichment, saves to JSON.
"""
...
LISTING_URL = "https://www.nysed.gov/funding-opportunities/grants"
OUTPUT_FILE = str(DATA_DIR / "ny_nysed_grants_raw.json")
...
GRANT_PATH_SEGMENTS = [
    "/funding-opportunities/", "/grants", "/funding", "/rfp", "/rfa", "/title-",
    "/program/", "/opportunity/", "/award",
    "/essa", "/idea", "/perkins", "/title-i", "/title-ii",
    "/library/", "/literacy", "/adult-education",
    "/early-childhood", "/special-education",
]

GRANT_TEXT_SIGNAL = {
    "grant", "fund", "rfp", "rfa", "award", "program", "opportunity",
    "title i", "title ii", "essa", "idea", "perkins",
    "education", "school", "literacy", "library", "adult",
    "early childhood", "special education", "workforce",
}

# Skip regulatory / informational pages
SKIP_PATH_SEGMENTS = [
    "/regulations/", "/policy/", "/forms/", "/data/",
    "/accountability/", "/testing/", "/certification/",
]
...
def scrape_listing() -> List[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "nysed.gov" not in parsed.netloc:
            continue

        path_lower = parsed.path.lower()
        if path_lower.rstrip("/") in ("", "/grants", "/funding-opportunities/grants",
                                       "/funding-opportunities"):
            continue
        if any(seg in path_lower for seg in SKIP_PATH_SEGMENTS):
            continue
...
def scrape_detail(link: Dict) -> Optional[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        href  = a["href"].strip()
        if any(kw in label for kw in ["apply", "application", "submit", "portal", "grants.gov", "nysed business portal"]):
...
def run(save_json: bool = True) -> List[Dict]:
    log.info("=" * 70)
    log.info(f"NY STATE EDUCATION DEPT GRANTS SCRAPER — {LISTING_URL}")
