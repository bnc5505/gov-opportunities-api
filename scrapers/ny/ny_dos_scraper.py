"""
New York Department of State — Community & Local Government Grants Scraper
Source: https://dos.ny.gov/funding-bid-opportunities

Scrapes DOS community development, local government, and coastal grants,
...
LISTING_URL = "https://dos.ny.gov/funding-bid-opportunities"
OUTPUT_FILE = str(DATA_DIR / "ny_dos_grants_raw.json")
...
SKIP_KEYWORDS  = {
    ...
    "press", "events", "faq", "about",
}

# DOS grant pages often live as root-level slugs (e.g., /2025-2026-lge-grant)
# so path segments alone are insufficient — text signal covers the rest
GRANT_PATH_SEGMENTS = [
    "/funding", "/grant", "/community-", "/local-government",
    "/coastal-", "/municipal-", "/neighborhood-", "/planning-",
    "/cdbg", "/lwrp", "/bwp", "/program", "/award",
]

GRANT_TEXT_SIGNAL = {
    "grant", "fund", "loan", "program", "award", "assistance",
    "community", "coastal", "municipal", "neighborhood", "planning",
    "local government", "development", "revitalization",
}
...
def scrape_listing() -> List[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if not parsed.netloc.endswith("dos.ny.gov"):
            continue

        path_lower = parsed.path.lower()
        if path_lower.rstrip("/") in ("", "/funding-bid-opportunities", "/funding"):
            continue
        if any(seg in path_lower for seg in ["/about/", "/contact/", "/news/", "/forms/", "/sitemap"]):
            continue
...
def run(save_json: bool = True) -> List[Dict]:
    log.info("=" * 70)
    log.info(f"NY DEPT OF STATE GRANTS SCRAPER — {LISTING_URL}")
    log.info("=" * 70)
    start = datetime.now()
