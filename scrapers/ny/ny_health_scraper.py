"""
New York State Department of Health — Grant Programs Scraper
Source: https://www.health.ny.gov/funding/

Scrapes DOH public health, disease prevention, and health systems grant
programs, follows each detail page, runs Azure AI enrichment, saves to JSON.
"""
...
LISTING_URL = "https://www.health.ny.gov/funding/"
OUTPUT_FILE = str(DATA_DIR / "ny_health_grants_raw.json")
...
GRANT_PATH_SEGMENTS = [
    "/funding/", "/grant", "/rfp", "/rfq", "/rfa",
    "/program/", "/opportunity", "/award",
    "/community-health", "/mental-health", "/maternal",
    "/prevention", "/chronic-disease", "/primary-care",
]

GRANT_TEXT_SIGNAL = {
    "grant", "fund", "rfp", "rfq", "rfa", "award", "program",
    "opportunity", "assistance", "public health", "prevention",
    "community", "health", "mental health", "maternal",
    "workforce", "clinic", "hospital", "medicaid",
}

# Skip pure informational sections
NAV_SLUGS = {
    "regulations", "laws", "statistics", "data", "reports",
    "permits", "licenses", "forms", "training",
}
...
def scrape_listing() -> List[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "health.ny.gov" not in parsed.netloc:
            continue

        path_lower = parsed.path.lower()
        if path_lower.rstrip("/") in ("", "/funding"):
            continue
...
def scrape_detail(link: Dict) -> Optional[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        href  = a["href"].strip()
        if any(kw in label for kw in ["apply", "application", "submit", "portal", "grants.gov", "rfp"]):
...
def run(save_json: bool = True) -> List[Dict]:
    log.info("=" * 70)
    log.info(f"NY DEPT OF HEALTH GRANTS SCRAPER — {LISTING_URL}")
    log.info("=" * 70)
    start = datetime.now()
