"""
New York Office of Children and Family Services — Grants Scraper
Source: https://ocfs.ny.gov/main/grants/

Scrapes OCFS child welfare, family support, and youth development grant
...
LISTING_URL = "https://ocfs.ny.gov/main/grants/"
OUTPUT_FILE = str(DATA_DIR / "ny_ocfs_grants_raw.json")
...
GRANT_PATH_SEGMENTS = [
    "/grants/", "/grant-", "/funding/", "/rfp", "/rfa",
    "/program/", "/opportunity/", "/award",
    "/child-welfare", "/family-", "/youth-",
    "/foster", "/adoption", "/prevention",
]

GRANT_TEXT_SIGNAL = {
    "grant", "fund", "rfp", "rfa", "award", "program", "opportunity",
    "assistance", "child", "family", "youth", "foster", "adoption",
    "prevention", "welfare", "services", "community",
}
...
def scrape_listing() -> List[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "ocfs.ny.gov" not in parsed.netloc:
            continue

        path_lower = parsed.path.lower()
        if path_lower.rstrip("/") in ("", "/main/grants"):
            continue
        if any(seg in path_lower for seg in ["/about/", "/contact/", "/news/", "/regulations/"]):
            continue
...
def run(save_json: bool = True) -> List[Dict]:
    log.info("=" * 70)
    log.info(f"NY OCFS GRANTS SCRAPER — {LISTING_URL}")
    log.info("=" * 70)
    start = datetime.now()
