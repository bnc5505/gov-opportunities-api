"""
New York Homes and Community Renewal — Grants & Funding Scraper
Source: https://hcr.ny.gov/funding-opportunities

Scrapes HCR affordable housing, community development, and infrastructure
funding programs, follows each detail page, runs Azure AI enrichment,
saves to JSON.
"""
...
STATE       = "NY"
BASE_URL    = "https://hcr.ny.gov"
LISTING_URL = "https://hcr.ny.gov/funding-opportunities"
OUTPUT_FILE = str(DATA_DIR / "ny_homes_grants_raw.json")
...
GRANT_PATH_SEGMENTS = [
    "/funding", "/grant", "/loan", "/program", "/award",
    "/housing", "/community-development", "/home-",
    "/affordable", "/rental", "/homeownership",
    "/weatherization", "/cdbg", "/htf", "/lihtc",
    "/infrastructure", "/rural",
]

GRANT_TEXT_SIGNAL = {
    "grant", "fund", "loan", "award", "program", "opportunity",
    "housing", "affordable", "community", "development",
    "rental", "homeownership", "weatherization", "rehab",
    "infrastructure", "rural", "cdbg", "home", "assistance",
}
...
def scrape_listing() -> List[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "hcr.ny.gov" not in parsed.netloc and "nyshcr.org" not in parsed.netloc:
            continue

        path_lower = parsed.path.lower()
        if path_lower.rstrip("/") in ("", "/funding-opportunities"):
            continue
        if any(seg in path_lower for seg in ["/about/", "/contact/", "/news/", "/reports/"]):
            continue
...
def scrape_detail(link: Dict) -> Optional[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        href  = a["href"].strip()
        if any(kw in label for kw in ["apply", "application", "submit", "portal", "grants.gov", "hcr online"]):
...
def run(save_json: bool = True) -> List[Dict]:
    log.info("=" * 70)
    log.info(f"NY HOMES AND COMMUNITY RENEWAL SCRAPER — {LISTING_URL}")
    log.info("=" * 70)
    start = datetime.now()
