"""
Empire State Development — Grants & Funding Scraper
Sources: https://esd.ny.gov (homepage) + key sub-sections

Scrapes ESD's broader funding opportunities (grants, loans, tax credits,
incentives) by scanning the homepage and known sub-sections, follows each
detail page, runs Azure AI enrichment, saves to JSON.
"""
...
OUTPUT_FILE = str(DATA_DIR / "ny_empire_grants_raw.json")

# ESD has no single grants index page — scan these entry points for grant links
LISTING_URLS = [
    "https://esd.ny.gov",
    "https://esd.ny.gov/doing-business-ny",
    "https://esd.ny.gov/tax-based-incentives",
    "https://esd.ny.gov/regional-economic-development-councils",
]

SKIP_FRAGMENTS = {"#", "javascript:", "mailto:", "tel:"}
SKIP_KEYWORDS  = {
    "login", "logout", "contact", "sitemap", "privacy", "accessibility",
    "facebook", "twitter", "linkedin", "youtube", "careers", "news",
    "press", "events", "faq", "about", "annual-report",
}

GRANT_PATH_SEGMENTS = [
    "/funding", "/grant", "/loan", "/incentive", "/tax-credit",
    "/program", "/assistance", "/opportunity", "/award",
    "/workforce", "/small-business", "/innovation",
    "/urban-", "/rural-", "/regional-",
]

GRANT_TEXT_SIGNAL = {
    "grant", "fund", "loan", "incentive", "tax credit", "program",
    "award", "assistance", "opportunity", "financing", "credit",
    "workforce", "business", "investment", "capital",
}
...
def _collect_links_from_page(listing_url: str, seen: set) -> List[Dict]:
    """Scrape one listing/entry page and return new grant links found."""
    log.info(f"  Scanning: {listing_url}")
    r = safe_get(listing_url)
    if not r:
        log.warning(f"  Could not fetch {listing_url}")
        return []
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "esd.ny.gov" not in parsed.netloc and "empire.state.ny.us" not in parsed.netloc:
            continue

        path_lower = parsed.path.lower()
        if path_lower.rstrip("/") in ("", "/doing-business-ny",
                                       "/tax-based-incentives",
                                       "/regional-economic-development-councils"):
            continue
        if any(seg in path_lower for seg in ["/news/", "/press-", "/about/", "/contact/",
...
def scrape_listing() -> List[Dict]:
    log.info(f"LAYER 1 — Multi-page scan of ESD ({len(LISTING_URLS)} entry points)")
    links = []
    seen  = set()

    for listing_url in LISTING_URLS:
        new_links = _collect_links_from_page(listing_url, seen)
        links.extend(new_links)

    log.info(f"LAYER 1 complete — {len(links)} grant pages found across all entry points")
    return links
...
def scrape_detail(link: Dict) -> Optional[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        href  = a["href"].strip()
        if any(kw in label for kw in ["apply", "application", "submit", "portal", "grants.gov"]):
...
def run(save_json: bool = True) -> List[Dict]:
    log.info("=" * 70)
    log.info(f"NY EMPIRE STATE DEVELOPMENT SCRAPER — {', '.join(LISTING_URLS)}")
    log.info("=" * 70)
    start = datetime.now()
