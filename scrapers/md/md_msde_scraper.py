"""
Maryland State Department of Education — Grant Programs Scraper
Source: https://marylandpublicschools.org/about/pages/ofpos/gac/grantprograms/index.aspx

Scrapes the full MSDE grant listing, follows each detail page,
extracts all grant fields, runs Azure AI enrichment, then loads to DB.
"""
...
STATE       = "MD"
BASE_URL    = "https://marylandpublicschools.org"
LISTING_URL = "https://marylandpublicschools.org/about/pages/ofpos/gac/grantprograms/index.aspx"
OUTPUT_FILE = str(DATA_DIR / "md_msde_grants_raw.json")

SKIP_FRAGMENTS = {"#", "javascript:", "mailto:", "tel:"}
SKIP_KEYWORDS  = {
    "login", "contact-us", "sitemap", "privacy", "accessibility",
...
def scrape_listing() -> List[Dict]:
    """Fetch main listing page, return all grant detail page links."""
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "marylandpublicschools.org" not in parsed.netloc:
            continue

        path_lower = parsed.path.lower()
        if not any(seg in path_lower for seg in ["/gac/", "/dsfss/", "/dcaa/", "/multilingual"]):
            continue

        if "grantprograms/index" in path_lower:
            continue
...
def scrape_detail(link: Dict) -> Optional[Dict]:
    """Fetch one grant detail page; extract HTML text and PDF links."""
    url  = link["url"]
    name = link["name"]
    ...
    for a in soup.find_all("a", href=True):
        ...
        href  = a["href"].strip()
        if any(kw in label for kw in ["apply", "application", "submit", "egrants", "smarterselect", "grants.gov"]):
...
def process_pdfs(page: Dict) -> Dict:
    """Download and extract text from every PDF found on the page."""
    pdf_extractions = []
...
def merge(page: Dict) -> Dict:
    """Best-wins merge of HTML and PDF extracted fields."""
...
def ai_pass(merged: Dict) -> Dict:
    """Send combined text to Azure AI for structured extraction."""
    ...
    # Keep regex wins over AI if they exist
...
def run(save_json: bool = True) -> List[Dict]:
    log.info("=" * 70)
    log.info(f"MD MSDE SCRAPER — {LISTING_URL}")
    log.info("=" * 70)
    start = datetime.now()
