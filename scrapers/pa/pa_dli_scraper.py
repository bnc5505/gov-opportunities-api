"""
Pennsylvania Department of Labor & Industry — Workforce Grants Scraper
Source: https://www.dli.pa.gov/Businesses/Workforce-Development/Pages/Grants.aspx

Scrapes DLI's workforce grant programs, follows each detail page,
extracts all grant fields, runs Azure AI enrichment, then saves to JSON.
"""
...
LISTING_URL = "https://www.dli.pa.gov/Businesses/Workforce-Development/Pages/Grants.aspx"
OUTPUT_FILE = str(DATA_DIR / "pa_dli_grants_raw.json")

SKIP_FRAGMENTS = {"#", "javascript:", "mailto:", "tel:"}
SKIP_KEYWORDS  = {
    "login", "logout", "contact-us", "sitemap", "privacy", "accessibility",
    "facebook", "twitter", "linkedin", "youtube", "careers", "news", "events",
}

# paths that indicate a real grant/workforce program on dli.pa.gov
GRANT_PATH_SEGMENTS = [
    "/grants/", "/grant-", "/funding/", "/fund-",
    "/workforce-innovation", "/wioa", "/rapid-response",
...
# anchor text must include at least one of these
GRANT_TEXT_SIGNAL = {
    "grant", "fund", "loan", "program", "award", "assistance", "incentive",
    "scholarship", "tax credit", "apprenticeship", "workforce", "wioa",
    "rapid response", "layoff", "reimbursement", "relief",
}

# pure nav slugs to skip by last path segment
NAV_SLUGS = {
    "services", "agencies", "programs-services", "workforce-development-home",
    "americorps-in-pennsylvania", "pa-careerlink-", "financial-aid",
    "grant-opportunities", "home----------", "resources", "dli",
    "governor", "ltgovernor",
}
...
def scrape_listing() -> List[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "dli.pa.gov" not in parsed.netloc:
            continue

        path_lower = parsed.path.lower()
        if path_lower.rstrip("/") in ("", "/businesses/workforce-development/pages/grants"):
            continue
        if any(kw in path_lower for kw in ["/pages/default", "/pages/home", "/about", "/forms"]):
            continue

        slug = path_lower.rstrip("/").split("/")[-1]
        if any(slug.startswith(nav) for nav in NAV_SLUGS):
            continue
...
def scrape_detail(link: Dict) -> Optional[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        href  = a["href"].strip()
        if any(kw in label for kw in ["apply", "application", "submit", "portal", "egrants", "smarterselect"]):
...
def run(save_json: bool = True) -> List[Dict]:
    log.info("=" * 70)
    log.info(f"PA DLI WORKFORCE GRANTS SCRAPER — {LISTING_URL}")
    log.info("=" * 70)
    start = datetime.now()
