"""
PennVEST — Pennsylvania Water & Infrastructure Funding Scraper
Source: https://www.pennvestinvestments.com/

Scrapes PennVEST's loan and grant programs for water, wastewater,
stormwater and non-point source pollution projects. Follows each
program detail page, runs Azure AI enrichment, saves to JSON.
"""
...
STATE       = "PA"
BASE_URL    = "https://www.pennvestinvestments.com"
LISTING_URL = "https://www.pennvestinvestments.com/"
OUTPUT_FILE = str(DATA_DIR / "pa_pennvest_grants_raw.json")
...
# anchor text or URL must contain one of these
GRANT_TEXT_KEYWORDS = {
    "grant", "loan", "fund", "program", "financing", "assistance",
    "water", "wastewater", "stormwater", "infrastructure", "project",
...
def scrape_listing() -> List[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "pennvestinvestments.com" not in parsed.netloc:
            continue
        ...
        has_grant_kw = any(kw in text_lower or kw in path_lower for kw in GRANT_TEXT_KEYWORDS)
        if not has_grant_kw:
            continue
...
def run(save_json: bool = True) -> List[Dict]:
    log.info("=" * 70)
    log.info(f"PENNVEST WATER & INFRASTRUCTURE SCRAPER — {LISTING_URL}")
    log.info("=" * 70)
    start = datetime.now()
