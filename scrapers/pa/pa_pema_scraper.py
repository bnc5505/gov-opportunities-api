"""
Pennsylvania Emergency Management Agency — Grants Scraper
Source: https://www.pema.pa.gov/Grants/Pages/default.aspx

Scrapes PEMA's federal and state emergency management grant programs
(FEMA pass-throughs, Hazard Mitigation, BRIC, etc.), follows each
...
STATE       = "PA"
BASE_URL    = "https://www.pema.pa.gov"
LISTING_URL = "https://www.pema.pa.gov/Grants/Pages/default.aspx"
OUTPUT_FILE = str(DATA_DIR / "pa_pema_grants_raw.json")
...
def scrape_listing() -> List[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "pema.pa.gov" not in parsed.netloc:
            continue
...
def scrape_detail(link: Dict) -> Optional[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        href  = a["href"].strip()
        if any(kw in label for kw in ["apply", "application", "submit", "portal", "grants.gov", "fema"]):
...
def run(save_json: bool = True) -> List[Dict]:
    log.info("=" * 70)
    log.info(f"PA PEMA EMERGENCY MANAGEMENT GRANTS SCRAPER — {LISTING_URL}")
    log.info("=" * 70)
    start = datetime.now()
