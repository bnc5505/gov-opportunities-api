"""
Pennsylvania Dept of Conservation & Natural Resources — Grants Scraper
Source: https://www.dcnr.pa.gov/Grants/Pages/default.aspx

Scrapes DCNR's conservation, recreation, and community grants,
follows each program detail page, extracts all fields, runs Azure AI
enrichment, then saves to JSON.
"""
...
STATE       = "PA"
BASE_URL    = "https://www.dcnr.pa.gov"
LISTING_URL = "https://www.dcnr.pa.gov/Grants/Pages/default.aspx"
OUTPUT_FILE = str(DATA_DIR / "pa_dcnr_grants_raw.json")

SKIP_FRAGMENTS = {"#", "javascript:", "mailto:", "tel:"}
SKIP_KEYWORDS  = {
    "login", "logout", "contact", "sitemap", "privacy", "accessibility",
    "facebook", "twitter", "linkedin", "youtube", "careers", "news", "press",
    "events", "faq", "about-dcnr",
}
...
def scrape_listing() -> List[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "dcnr.pa.gov" not in parsed.netloc:
            continue

        path_lower = parsed.path.lower()
        if path_lower.rstrip("/") in ("", "/grants/pages/default", "/grants"):
            continue
        if "/pages/home" in path_lower or "/about" in path_lower:
            continue

        # Only follow pages within the /Grants/ section — skip recreation/nav links
...
def scrape_detail(link: Dict) -> Optional[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        href  = a["href"].strip()
        if any(kw in label for kw in ["apply", "application", "submit", "portal", "egrants", "grants.gov"]):
...
def run(save_json: bool = True) -> List[Dict]:
    log.info("=" * 70)
    log.info(f"PA DCNR GRANTS SCRAPER — {LISTING_URL}")
    log.info("=" * 70)
    start = datetime.now()
