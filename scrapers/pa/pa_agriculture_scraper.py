"""
Pennsylvania Department of Agriculture — Grants Scraper
Source: https://www.agriculture.pa.gov/Grants/Pages/default.aspx

Scrapes PDA's agriculture, rural, and food-system grant programs,
follows each detail page, runs Azure AI enrichment, saves to JSON.

Note: pasda.pa.gov is the PA Spatial Data Access GIS portal — not a grants
site. PDA grant programs are on agriculture.pa.gov.
"""
...
STATE       = "PA"
BASE_URL    = "https://www.agriculture.pa.gov"
LISTING_URL = "https://www.agriculture.pa.gov/Grants/Pages/default.aspx"
OUTPUT_FILE = str(DATA_DIR / "pa_agriculture_grants_raw.json")
...
# agriculture.pa.gov global nav pollutes the listing — only follow these sub-trees
GRANT_PATH_SEGMENTS = [
    "/business-and-industry/",
    "/plants-land-water/farmland-preservation",
    "/plants-land-water/plant-industry/",
    "/food-systems/",
    "/food/food-safety/assistance",
    "/agencies/pda/plants-land-water/spotted-lanternfly/",
    "/grants/",
    "/pa-fair-fund/",
    "/hardwoods-",
...
# anchor text must contain at least one of these for borderline paths
GRANT_TEXT_SIGNAL = {
    "grant", "fund", "loan", "program", "award", "assistance", "incentive",
    "scholarship", "rebate", "reimbursement", "credit", "subsidy", "relief",
    "initiative", "preservation", "conservation", "development", "investment",
    "tax", "financing",
}
...
def scrape_listing() -> List[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "agriculture.pa.gov" not in parsed.netloc:
            continue

        path_lower = parsed.path.lower()
        if path_lower.rstrip("/") in ("", "/grants/pages/default", "/grants"):
            continue
        if "/pages/home" in path_lower:
            continue
        if any(seg in path_lower for seg in [
            "/about-pda", "/about-the-", "/agencies/", "/animals/",
            "/consumer-protection/", "/boards-commissions", "/bureau-directors",
            "/open-jobs", "/budget", "/plants-land-water/plant-industry/agr",
        ]):
            continue
...
def scrape_detail(link: Dict) -> Optional[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        href  = a["href"].strip()
        if any(kw in label for kw in ["apply", "application", "submit", "portal", "grants.gov", "egrants"]):
...
def run(save_json: bool = True) -> List[Dict]:
    log.info("=" * 70)
    log.info(f"PA DEPARTMENT OF AGRICULTURE GRANTS SCRAPER — {LISTING_URL}")
    log.info("=" * 70)
    start = datetime.now()
