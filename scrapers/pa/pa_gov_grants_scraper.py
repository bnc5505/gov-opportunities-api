"""
Pennsylvania Official Grants Directory Scraper
Source: https://www.pa.gov/guides/grants/

Scrapes the PA.gov grants guide, follows each grant detail link,
...
LISTING_URL = "https://www.pa.gov/guides/grants/"
OUTPUT_FILE = str(DATA_DIR / "pa_gov_grants_raw.json")

SKIP_FRAGMENTS = {"#", "javascript:", "mailto:", "tel:"}
SKIP_KEYWORDS  = {
    "login", "logout", "contact", "sitemap", "privacy", "accessibility",
    "facebook", "twitter", "linkedin", "youtube", "instagram",
    "careers", "jobs", "news", "press", "events", "faq",
}
...
def scrape_listing() -> List[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        is_pa_gov  = "pa.gov" in parsed.netloc
        has_grant  = any(seg in path_lower for seg in GRANT_PATH_KEYWORDS)

        if is_pa_gov and not has_grant:
            continue

        if path_lower.rstrip("/") in ("", "/guides", "/guides/grants"):
            continue
...
def scrape_detail(link: Dict) -> Optional[Dict]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        href  = a["href"].strip()
        if any(kw in label for kw in ["apply", "application", "submit", "portal", "egrants"]):
...
def run(save_json: bool = True) -> List[Dict]:
    log.info("=" * 70)
    log.info(f"PA GOV GRANTS SCRAPER — {LISTING_URL}")
    log.info("=" * 70)
    start = datetime.now()
